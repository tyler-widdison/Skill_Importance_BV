...
winners <- 
  df %>% 
  filter(name != '', gender == 'W', star %in% c('4*', '5*', 'WC', 'WTF'), phase %in% c('Pool B','Round 2','Pool C','Round 1','Final 1st Place','Round 3','Pool G','Pool D','Pool F','Semifinals','Pool E','Pool A','Final 3rd Place','Pool H','Finals 1st Place','Finals 3rd Place','Round of 32','Round of 16','Quarterfinals','Pool L','Pool I','Pool J','Pool K','Finals')) %>%
...
# Women's rally ending skill exploration

This data set includes 918 matches, played in main draw, from the 2019 season (HAG 2019 - CHE 2019). Rally ending skills for 4 star, 5 star, World Championships and World Tour Final. 

```{r Womens skill importance EDA, echo=FALSE, fig.width=8, fig.height=6}
vb_df %>%
  ...
  ggtitle("Rally ending skills vs teams winning or losing in 2019-2020 Women's Main Draw matches")
```
...
```{r Womens rally ending skill importance, echo=FALSE, fig.width=8, fig.height=6}
set.seed(123)
...
```{r EDA of atk kill & atk error, echo=FALSE, fig.width=8, fig.height=6}

df2$date <- as.Date(df2$date)
df2$team_finish <- as.integer(as.character(df2$team_finish))

df2 %>% 
  pivot_longer(cols = c('Attack_Kill','Attack_Error')) %>% 
...
df2 %>% 
  ...
  group_by(date, team_finish, match_id) %>% summarise(Attack_Error = sum(Attack_Error), Attack_Kill = sum(Attack_Kill)) %>%
  pivot_longer(cols = c('Attack_Kill','Attack_Error')) %>% 
    ...
    geom_hline(yintercept = 28.4, color = 'cyan3', linetype = 'dashed') + xlab('Date') + ylab('Number of Attack Errors or Attack Kills') + 
    geom_hline(yintercept = 3.68, color = 'salmon', linetype = 'dashed')+theme(legend.title=element_blank())
```
